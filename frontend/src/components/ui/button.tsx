...
const buttonVariants = cva(
  // Pill geometry + a soft press: the modern reference designs round every
  // action control fully and let colour, not borders, carry the hierarchy.
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full font-medium transition-[background-color,border-color,color,box-shadow,transform] active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0',
  {
    variants: {
      ...
      size: {
        xs: 'h-6 px-2.5 text-[length:var(--text-2xs)] [&_svg]:size-3',
        sm: 'h-7 px-3 text-[length:var(--text-xs)] [&_svg]:size-3.5',
        md: 'h-8 px-3.5 text-[length:var(--text-sm)] [&_svg]:size-4',
        lg: 'h-10 px-5 text-[length:var(--text-base)] [&_svg]:size-4',
